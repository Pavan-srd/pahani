...
<head>
  ...
  <title>{{ $pahani->document_name ?? 'Document' }} — Cloudflare Viewer</title>
  <style>
...
<body oncontextmenu="return false" ondragstart="return false">
  ...
  <div class="gov-header">
    <div class="gov-top-bar">
      ...
      <span>Secure Document Viewer (Cloudflare R2)</span>
    </div>
  ...
  <div class="cloudflare-bar">
    Document is securely delivered via Cloudflare R2 (no download to Laravel server)
  </div>
  ...
  <div class="site-footer">
    ...
    <div class="cloudflare-note">
      PDF loaded directly from Cloudflare R2 • Signed URL expires in 60 minutes • No data stored on Laravel server
    </div>
  </div>
  ...
  <script>
    /*
    ════════════════════════════════════════════════════════════════════════════
    SECURE PDF VIEWER — DIRECT CLOUDFLARE R2 URL LOADING
    ════════════════════════════════════════════════════════════════════════════
    
    ARCHITECTURE:
    - PDF URL: Signed Cloudflare R2 URL (passed from Laravel controller)
    - Loading: PDF.js loads directly from Cloudflare (NOT through Laravel)
    - Bandwidth: Cloudflare handles file delivery (efficient for 150-200MB+ files)
    - Memory: Zero buffering in Laravel (file stays in Cloudflare)
    ...
    4. Server-side auth: Laravel checks login before generating URL
    5. Server-side authz: Laravel checks user permission before generating URL
    ...
    */
     ...
     * - Laravel verified user permission
     * - Cloudflare verifies signature
     * - Cloudflare checks expiration
     * - Direct file delivery from Cloudflare
     */
    pdfjsLib.getDocument(PDF_SOURCE_URL).promise
      ...
      .catch(error => {
        ...
        if (error.message && error.message.includes('404')) {
          showError('PDF not found in Cloudflare R2. It may have been removed.');
