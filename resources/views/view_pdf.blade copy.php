...
/*
  DEMO DATA — replace with real server data (e.g. via @json($records) in Blade).
  Structure: { [mandal_village_key]: { [docValue]: { fileName, fileUrl } } }
  fileUrl should be the path to the PDF served by Laravel, e.g. /storage/pahani/...
*/
const UPLOADED_RECORDS = {
  'hayathnagar__hayathnagar': {
    'sethwar': { fileName:'Sethwar_Hayathnagar_2024.pdf',  fileUrl: '' },
    'kasra':   { fileName:'Kasra_Hayathnagar_2024.pdf',    fileUrl: '' },
    'yr_1972': { fileName:'Pahani_1972-73_Hayathnagar.pdf',fileUrl: '' },
    'yr_1985': { fileName:'Pahani_1985-86_Hayathnagar.pdf',fileUrl: '' },
    'yr_2001': { fileName:'Pahani_2001-02_Hayathnagar.pdf',fileUrl: '' },
    'yr_2015': { fileName:'Pahani_2015-16_Hayathnagar.pdf',fileUrl: '' },
    'yr_2023': { fileName:'Pahani_2023-24_Hayathnagar.pdf',fileUrl: '' }
  },
  'keesara__ghatkesar': {
    'sethwar': { fileName:'Sethwar_Ghatkesar.pdf', fileUrl: '' },
    'yr_1990': { fileName:'Pahani_1990-91_Ghatkesar.pdf', fileUrl: '' },
    'yr_2010': { fileName:'Pahani_2010-11_Ghatkesar.pdf', fileUrl: '' }
  }
  ...
  */
  if (opt.fileUrl) {
    ...
    frame.srcdoc =
      ...
      '.icon{font-size:48px;margin-bottom:16px}h2{font-size:16px;color:#f39c12;margin-bottom:8px}p{font-size:12px;color:#ccc;max-width:320px}' +
      '</style></head><body>' +
      '<div class="box"><div class="icon">&#128196;</div>' +
      '<h2>' + opt.title + '</h2>' +
      '<p style="color:#aaa;font-size:11px">' + opt.fileName + '</p>' +
      '<p>This is a demo placeholder.<br>In production, set <code style="background:#333;padding:2px 5px;border-radius:2px;color:#f39c12">fileUrl</code> in <code style="background:#333;padding:2px 5px;border-radius:2px;color:#f39c12">UPLOADED_RECORDS</code> to the real Laravel storage path.</p>' +
      '</div></body></html>';
  }
