...
<head>
  ...
  <title>Mandal Management — Admin Dashboard</title>
  <style>
  ...
  <div class="app-shell">
    ...
    <aside class="sidebar" id="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        <a class="sb-item active" href="{{ route('admin.mandals.index') }}">
    ...
    <div class="main-col">
      ...
      <div class="topbar">
        <div class="topbar-left">
          ...
          <div>
            <div class="topbar-title">🏛️ Mandal Management</div>
            <div class="topbar-breadcrumb">Admin › Master Data › Mandal</div>
          </div>
        </div>
      ...
      <div class="content">
        ...
        <div class="toolbar">
          <div class="search-box">
            🔎 <input type="text" id="mandal-search" placeholder="Search Mandal by name…" oninput="filterAndPaginate()">
          </div>
        ...
        <div class="section-card">
          ...
          <div class="pagination-container" id="mandal-pagination">
      ...
      <form id="mandal-add-form" onsubmit="submitMandalForm(event)">
        <div class="modal-body">
          <div class="form-field" id="mandal-name-field">
            ...
            <input type="text" id="mandal-name-input" placeholder="e.g., Sangareddy" required>
      ...
      <form id="mandal-edit-form" onsubmit="submitEditMandalForm(event)">
        ...
        <div class="modal-body">
          <div class="form-field" id="mandal-edit-name-field">
            <label>Mandal Name *</label>
            <input type="text" id="mandal-edit-name-input" placeholder="e.g., Sangareddy" required>
...
/* ══════════════════════════════════════════════════════════════════
   LOAD MANDAL DATA
══════════════════════════════════════════════════════════════════ */
function loadMandalData() {
  const loadingEl = document.getElementById('mandal-loading');
  const tableEl   = document.getElementById('mandal-table');
  const emptyEl   = document.getElementById('mandal-empty');
  ...
  fetch('/api/admin/mandals', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
    ...
    .catch(() => showToast('Failed to load mandals.', true))
...
function renderTable() {
  const tbody   = document.getElementById('mandal-tbody');
  const tableEl = document.getElementById('mandal-table');
  const emptyEl = document.getElementById('mandal-empty');
    ...
    tr.innerHTML = `
      ...
      <td style="text-align:center">
        <div class="row-actions" style="justify-content:center">
          <button class="btn-icon" title="Edit" onclick="openEditMandalModal(${row.id})">✎</button>
...
function updatePagination() {
  const state = paginationState;
  const paginationEl = document.getElementById('mandal-pagination');
  const navEl = document.getElementById('mandal-nav');
  ...
  document.getElementById('mandal-info-from').textContent = start;
  document.getElementById('mandal-info-to').textContent = end;
  document.getElementById('mandal-info-total').textContent = state.filteredData.length;
...
function filterAndPaginate() {
  ...
  const query = document.getElementById('mandal-search').value.toLowerCase();
  paginationState.filteredData = paginationState.allData.filter(m =>
    m.name.toLowerCase().includes(query) || m.slug.toLowerCase().includes(query)
  );
  paginationState.currentPage = 1;
  paginationState.totalPages = Math.ceil(paginationState.filteredData.length / PAGINATION_SIZE) || 1;
  renderTable();
...
/* ══════════════════════════════════════════════════════════════════
   EDIT MANDAL
══════════════════════════════════════════════════════════════════ */
function openEditMandalModal(id) {
  const mandal = paginationState.allData.find(m => m.id === id);
  if (!mandal) return;

  document.getElementById('mandal-edit-id').value = id;
  document.getElementById('mandal-edit-name-input').value = mandal.name;
  clearFieldError('mandal-edit-name-field', 'mandal-edit-name-error');
  openModal('mandal-edit-modal-overlay');
  document.getElementById('mandal-edit-name-input').focus();
}
...
function openDeleteModal(id) {
  ...
  pendingDelete = { type: 'mandal', id: id, name: mandal.name };
  document.getElementById('delete-modal-message').textContent = `Are you sure you want to delete the mandal "${mandal.name}"? This action cannot be undone.`;
...
function confirmDelete() {
  ...
  fetch(`/api/admin/mandals/${pendingDelete.id}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
  ...
  .then(async r => {
    ...
    if (!r.ok || !data.success) {
      showToast(data.message || 'Failed to delete mandal.', true);
      return;
    }
